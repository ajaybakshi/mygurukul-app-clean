 ...
 * Tests the randomization behavior of the Today's Wisdom API
 * Verifies source selection, verse extraction, and content diversity
 */
...
const testRandomization = async () => {
  ...
  console.log('\n📋 DETAILED RESULTS:');
  console.log('Test | Source | Method | Category | TextType | Content Preview');
  console.log('-----|--------|--------|----------|----------|----------------');
  
  results.forEach(result => {
    if (result.error) {
      console.log(`${result.testNumber.toString().padStart(4)} | ERROR | ERROR | ERROR | ERROR | ${result.error}`);
    } else {
      console.log(
        `${result.testNumber.toString().padStart(4)} | ` +
        `${result.source.padEnd(6)} | ` +
        `${result.method.padEnd(6)} | ` +
        `${result.category.substring(0, 8).padEnd(8)} | ` +
        `${result.textType.substring(0, 8).padEnd(8)} | ` +

...
PROMPT_TEMPLATE = """You are a meticulous AI research assistant with deep expertise in the Śrīmad-Bhāgavatam (Bhagavata Purana), Puranic literature, Vaishnava philosophy, bhakti (devotion), and ancient Indian storytelling traditions. Working strictly from the PDF chapter provided, craft a single, well-formed JSON object that adheres to the existing metadata schema used across the library.

Your metadata must balance narrative richness with spiritual and philosophical insight so that readers seeking wisdom, devotional guidance, cultural understanding, and moral teachings can all benefit. Observe the following requirements:

1. "chapterTitle": Extract the exact chapter title as presented in the PDF (e.g., "SB 12.1- The Degraded Dynasties of Kali-yuga" or "The Appearance of Śrī Nārada"). Return a clean string. Include the canto and chapter numbers if present in the PDF.

2. "aiSummary": Deliver a 4-6 sentence synthesis highlighting: the main characters and their roles (deities, sages, devotees, or historical figures), the narrative progression or story elements, the spiritual lesson or philosophical teaching (dharma, bhakti, karma, moksha), and why this chapter matters for modern readers seeking wisdom, devotional guidance, or cultural understanding.

3. "keyConcepts": Provide 6-8 pivotal Sanskrit terms, spiritual principles, or narrative concepts discussed in the chapter. Each list item must be an object with "term" (IAST transliteration when possible, or English term) and "definition" (succinct, context-aware explanation). Prioritize vocabulary around bhakti (devotion), dharma (duty), karma (action), moksha (liberation), avatāra (incarnation), līlā (divine play), and other story-relevant spiritual concepts.

4. "searchTags": Supply 12-18 search tags as plain strings that broaden discovery. Mix at least three tags aimed at: (a) wisdom seekers and spiritual learners (e.g., "Devotional teachings", "Spiritual wisdom"), (b) devotees and practitioners (e.g., "Bhakti yoga", "Krishna stories"), and (c) general-interest readers (e.g., "Puranic stories", "Ancient Indian narratives", "Hindu mythology"). Include related Sanskrit terms, modern synonyms, and thematic tags when helpful.

5. "deeperInsights": An object with two sub-fields:
   * "philosophicalViewpoint": Note how the chapter frames spirituality, devotion, dharma, human nature, or divine play—reference bhakti, dharma, karma, moksha, avatāra, or practical wisdom when applicable.
   * "practicalAdvice": An array of 2-4 strings (NOT objects), each string distilling actionable wisdom from the chapter—e.g., guidance on devotion, ethical conduct, spiritual practice, or applying philosophical principles. Each string should be concise and faithful to the chapter's teaching. Example format: ["First piece of advice", "Second piece of advice", "Third piece of advice"].
...
- Focus on the chapter's narrative, spiritual teaching, and devotional aspects rather than technical treatises.
- When the chapter contains stories, emphasize the narrative elements, characters, and moral/spiritual lessons.
"""
...
def derive_title_from_filename(pdf_path: str) -> str:
    ...
    # Try parent folder name first (canto name)
    ...
    if parent_name and parent_name != pdf_path_obj.parent.parent.name:
        # Clean up canto folder names: "Canto_1_Srimad_Bhagvatam_Creation" -> "Canto 1: Creation"
        cleaned = parent_name.replace("_", " ")
        # Extract canto number and description
        canto_match = re.search(r"Canto[_\s]+(\d+)", cleaned, re.IGNORECASE)
        desc_match = re.search(r"Bhagvatam[_\s]+(.+)$", cleaned, re.IGNORECASE)
        if canto_match and desc_match:
            canto_num = canto_match.group(1)
            desc = desc_match.group(1).strip()
            return f"Canto {canto_num}: {desc}"
    ...
    # Clean up common patterns: "SB 12.1- The Degraded Dynasties" -> "The Degraded Dynasties"
    cleaned = filename.replace("_", " ")
    # Remove SB prefix and chapter numbers if present
    cleaned = re.sub(r"^SB\s*\d+\.\d+\s*[-–]\s*", "", cleaned, flags=re.IGNORECASE)
    cleaned = re.sub(r"^SB_\d+_\d+_", "", cleaned, flags=re.IGNORECASE)
    cleaned = re.sub(r"\s+", " ", cleaned).strip()
...
def should_process_pdf(pdf_path: Path, root_path: Path) -> bool:
    ...
    # Skip if PDF is directly in the Bhagvata Purana root directory
    if pdf_path.parent == root_path:
        return False
    
    # Additional check: skip known root-level files
    root_level_files = [
        "Bhagvata_Purana_Sanskrit.txt",
...
def process_directory(root_dir: str):
    ...
    print("=" * 80)
    print("ŚRĪMAD-BHĀGAVATAM METADATA GENERATOR")
    ...
    if not pdf_files:
        ...
        print("Note: Root-level PDFs (like Bhagvata_Purana_Sanskrit.txt) are skipped.")
        return

...
SCRIPTURE_ID = "manu_smriti"
SCRIPTURE_NAME = "Manu Smṛti"
SECTION_ID = "1"
SECTION_NAME = "Manu Smṛti Chapters"
GCS_BUCKET = "gs://mygurukul-sacred-texts-corpus"
GCS_BASE_PATH = "Gurukul_Library/Primary_Texts/Sastras/DharmaShastra/Manu_Smriti"
...
def get_chapter_number(path_name: str) -> int:
    """Extract numeric chapter identifier from folder or file name."""
...
def derive_fallback_title(folder_name: str) -> str:
    """Generate a readable fallback title from the chapter folder name."""

    cleaned = re.sub(r"^Manusmriti[_-]Buhler[_-]", "", folder_name, flags=re.IGNORECASE)
    cleaned = cleaned.replace("_", " ").replace("-", " ")
    return cleaned.strip() or folder_name


def chapter_directories(root_path: Path):
    """Yield chapter directories containing PDFs."""

    for directory in sorted(root_path.iterdir()):
        if not directory.is_dir():
            continue
        if "_Chapter_" not in directory.name:
            continue
...
def generate_chapter_manifest(root_directory: str):
    print("=" * 80)
    print("CHAPTER MANIFEST GENERATOR - MANU SMṚTI")
    print("=" * 80)
    ...
    directories = list(chapter_directories(root_path))
    if not directories:
...
def main():
    ...
    print("╔══════════════════════════════════════════════════════════╗")
    print("║                                                          ║")
    print("║    MYGURUKUL CHAPTER MANIFEST GENERATOR - MANU SMṚTI     ║")
    print("║                                                          ║")
    print("╚══════════════════════════════════════════════════════════╝")
    print("\n")
